...
function loadConfig(rulesPath: string): RouterConfig {
	if (!existsSync(rulesPath)) return defaultConfig();
	try {
		return JSON.parse(readFileSync(rulesPath, "utf-8")) as RouterConfig;
	} catch {
		console.warn("[model-router] failed to parse config, using defaults");
...
export default function (pi: ExtensionAPI) {
	...
	// session_start loads the config and stores it here so before_agent_start
	// (which fires immediately after) can reuse it without a second disk read.
	let configCache: RouterConfig | null = null;

	pi.on("before_agent_start", async (_event, ctx) => {
		const config = configCache ?? loadConfig(rulesPath);
		configCache = null;
		...
		async function setModelByTier(
			...
			if (modelName) {
				...
				if (model) {
					...
					ctx.ui.notify(
						`[model-router] model not found: ${modelName}`,
						"warning",
					);
				}
			}
		...
		const liveModel = ctx.model as { id?: string } | undefined;
		...
		if (isPinned(session)) {
			appendDecision(logPath, {
				...
				session,
				tier: "explicit",
				model: currentModelId(liveModel, config.defaultModel),
				reason: "explicit",
	...
	// Clear per-session state on shutdown to prevent unbounded map growth.
	...
	pi.on("session_start", (_event, ctx) => {
		configCache = loadConfig(rulesPath);
		const { ollamaUrl, ollamaModel } = configCache;
